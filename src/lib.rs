...
mod tests {
    ...
    use threshold_crypto::{
        PublicKey, PublicKeySet, PublicKeyShare, SecretKey, SecretKeySet, SecretKeyShare,
        SignatureShare,
    };
    ...
    fn signing_poc() {
        ...
        let res =
            <G1 as BLSSigCore<ExpandMsgXmd<sha2::Sha256>>>::core_verify(pk, sign, message, CSUITE);
    ...
    fn basics() {
        ...
        for key in &sk_set {
            ...
            let sig =
                <G1 as BLSSigCore<ExpandMsgXmd<sha2::Sha256>>>::core_sign(x_prime, message, CSUITE);
            signatures.push(sig);
        }

        // Verify signs
        for i in 0..ITERATIONS {
            let res = <G1 as BLSSigCore<ExpandMsgXmd<sha2::Sha256>>>::core_verify(
                pk_set[i],
                signatures[i],
                message,
                CSUITE,
        ...
        // Multi-sigs
        let aggregated_sign =
            <G1 as BLSSigCore<ExpandMsgXmd<sha2::Sha256>>>::aggregate(signatures.as_slice());
        let aggregated_pk =
            <G2 as BLSSigCore<ExpandMsgXmd<sha2::Sha256>>>::aggregate(pk_set.as_slice());
        assert!(<G1 as BLSSigCore<ExpandMsgXmd<sha2::Sha256>>>::core_verify(
            aggregated_pk,
            aggregated_sign,
            message,
            CSUITE
        ...
        for i in 0..ITERATIONS {
            ...
            let signature = <G1 as BLSSigCore<ExpandMsgXmd<sha2::Sha256>>>::core_sign(
                x_prime,
                &msg.as_bytes(),
                CSUITE,
            );
        ...
        assert!(
            <G1 as BLSSigCore<ExpandMsgXmd<sha2::Sha256>>>::core_aggregate_verify(
                pk_set.as_slice(),
                messages.as_slice(),
                asig,
                CSUITE
    ...
    fn threshold() {
        ...
        let start = Instant::now();
        let sig = pk_set.combine_signatures(sig_shares).unwrap();
